...
function loadSelectedCategory() {
  chrome.storage.local.get(["wikiRandomCategory"], (result) => {
    if (result.wikiRandomCategory) {
      initializeCategory(result.wikiRandomCategory) // initialise the cache for the stored category
    }
  });
}
...
const initializeCategory = async (value) => {
...
async function handleRandomArticle() {
  ...
  try {
    ...
    if (pagesList.length) {
      ...
      console.log("No pages fetched this round")
    }
